...
ssh amnonam@sheshet.cslab.openu.ac.il "ssh login.hpc.pub.lan mkdir work/$DIR_NAME"
# ssh amnonam@sheshet.cslab.openu.ac.il "ssh my.hpc.pub.lan mkdir work/$DIR_NAME"

# copy the SraRunTable to the openu server
rsync -avzP -e 'ssh -o "ProxyCommand ssh amnonam@sheshet.cslab.openu.ac.il -W %h:%p"' SraRunTable.txt amnonam@login.hpc.pub.lan:work/$DIR_NAME/
# copy to sheshet
...
ssh amnonam@sheshet.cslab.openu.ac.il "ssh login.hpc.pub.lan \"cd work/$DIR_NAME ; /home/amnonam/scripts/doit2.sh $DIR_NAME $@\"" 
